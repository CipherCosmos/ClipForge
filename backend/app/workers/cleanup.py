    ...
    try:
        ...
        for job in stale:
            ...
            if age > timedelta(minutes=JOB_TIMEOUT_MINUTES):
                ...
                logger.warning("Timed out stale job %s (%s, running for %d min)",
                               job.id, job.type.value, age.total_seconds() // 60)
            else:
...
def unstuck_pipeline():
    ...
    try:
        ...
        for video in stuck_videos:
            ...
            active_jobs = [j for j in jobs if j.status in (JobStatusEnum.RUNNING, JobStatusEnum.QUEUED)]
            ...
            if not active_jobs:
                ...
                if not done_jobs and not has_segments:
                    ...
                    logger.info("Unstuck video %s — re-enqueuing NLP + scene detect", str(video.id)[:8])
    except Exception as e:
