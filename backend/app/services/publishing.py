...
async def _resolve_token(
    token: str, platform: str, client_id: str | None = None, client_secret: str | None = None
) -> str:
    ...
    if platform == "youtube_shorts" and any(token.startswith(p) for p in _REFRESH_TOKEN_PREFIXES):
        ...
        if not cid or not csec:
            raise ValueError(
                "YouTube refresh token requires Client ID and Client Secret. "
                "Add them in Settings → Connected Accounts."
            )
        try:
...
async def publish_clip(
    ...
    try:
        ...
        tags = " ".join(t.strip().lstrip("#") for t in re.split(r"[\s,]+", hashtags) if t.strip())

        async with httpx.AsyncClient(timeout=120.0) as client:
            if platform == "youtube_shorts":
                return await _publish_youtube(
                    client, cfg, tmp.name, title, description, tags, resolved_token, privacy
                )
            elif platform == "tiktok":
                return await _publish_tiktok(
                    client, cfg, tmp.name, title, description, tags, resolved_token
                )
...
async def _publish_youtube(
    client, cfg, file_path, title, description, tags, token, privacy="public"
):
    ...
    try:
        with open(file_path, "rb") as f:
            ...
            short_title = (
                (title[:80] + " #Shorts") if not title.lower().endswith("#shorts") else title[:100]
            )
            ...
            if resp.status_code in (200, 201):
                try:
                    ...
                    return {
                        "success": False,
                        "error": f"YouTube returned {resp.status_code} with non-JSON response",
                    }
            elif resp.status_code in (401, 403):
                return {
                    "success": False,
                    "error": (
                        "YouTube rejected the access token. It may be expired or invalid. "
                ...
                try:
                    ...
                    return {
                        ...
                        "error": result.get("error", {}).get(
                            "message", f"YouTube API error ({resp.status_code}): {body_text}"
                        ),
                    }
                except Exception:
                    return {
                        "success": False,
                        "error": f"YouTube API returned {resp.status_code}: {body_text}",
                    }
    except Exception as e:
...
async def _publish_tiktok(client, cfg, file_path, title, description, tags, token):
    ...
    try:
        ...
        return {
            "success": True,
            "platform_url": "",
            "platform_id": init_data.get("data", {}).get("publish_id", ""),
        }
    except Exception as e:
        return {"success": False, "error": str(e)}
...
async def _publish_linkedin(client, cfg, file_path, title, description, tags, token):
    ...
    try:
        ...
        if register_resp.status_code not in (200, 201):
            return {
                "success": False,
                "error": f"LinkedIn upload registration failed: {register_resp.text}",
            }
        ...
        if not upload_url or not video_urn:
            return {
                "success": False,
                "error": "LinkedIn upload URL or URN missing from registration response",
            }
        ...
        with open(file_path, "rb") as f:
            ...
            if upload_resp.status_code not in (200, 201):
                return {
                    "success": False,
                    "error": f"LinkedIn binary upload failed: {upload_resp.text}",
                }
