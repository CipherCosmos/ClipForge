...
class TestSceneDetectViralScore:
    ...
    def test_single_dimension_at_one_returns_its_weight(self):
        seg = {"hook_score": 1.0}
        assert _calculate_viral_score(seg) == 0.25

    def test_multiple_single_dimensions(self):
        assert _calculate_viral_score({"emotion_intensity": 1.0}) == 0.20
        assert _calculate_viral_score({"engagement_potential": 1.0}) == 0.15
        assert _calculate_viral_score({"keyword_density": 1.0}) == 0.10
        assert _calculate_viral_score({"scene_change_intensity": 1.0}) == 0.10
        assert _calculate_viral_score({"audio_event_score": 1.0}) == 0.10
        assert _calculate_viral_score({"audio_energy": 1.0}) == 0.05
        assert _calculate_viral_score({"speaker_confidence": 1.0}) == 0.05
