...
def test(method: str, path: str, token: str | None = None,
         json_body: dict | None = None, params: dict | None = None,
         expect: int | list[int] = 200, label: str = ""):
...
def main():
    global passed, failed
    print(f"\n{'='*70}")
    print(f"  ClipForge — Complete API Endpoint Test")
    print(f"  Server: {BASE}")
    print(f"{'='*70}\n")
    ...
    # ── Auth (no auth) ──
    print("\n── Auth (Public) ──")
    test("POST", "/api/auth/register", json_body={"email": "test-endpoints@test.com", "password": "TestPass123!"}, expect=[200, 201, 409])
    test("POST", "/api/auth/login", json_body={"email": "test-endpoints@test.com", "password": "TestPass123!"},
    ...
    r = httpx.post(f"{API}/auth/login", json={"email": "test-endpoints@test.com", "password": "TestPass123!"}, timeout=10)
    ...
    test("POST", "/api/auth/refresh", json_body={"refresh_token": refresh_token}, expect=[200, 401], label="/auth/refresh")
    ...
    test("DELETE", f"/api/keys/{uuid.uuid4()}", token, expect=[401, 403, 404],
    ...
    test("POST", "/api/billing/cancel", token, expect=[200, 400, 501],
         label="/billing/cancel (expect 200, 400, or 501 if no Stripe)")

    # ── Videos ──
    print("\n── Videos ──")
    test("GET", "/api/videos", token)
    test("GET", f"/api/videos/{uuid.uuid4()}", token, expect=404,
         label="/videos/{id} (non-existent)")
    test("DELETE", f"/api/videos/{uuid.uuid4()}", token, expect=404,
         label="/videos/{id} (delete non-existent)")
    test("POST", "/api/videos/batch-delete", token, json_body={"ids": [str(uuid.uuid4())]},
         expect=[200, 204, 207], label="/videos/batch-delete")
    test("POST", "/api/videos/import", token, json_body={
    ...
    vid = str(uuid.uuid4())
    print("\n── Transcript ──")
    test("GET", f"/api/videos/{vid}/transcript", token, expect=404,
         label=f"/api/videos/{{id}}/transcript (no video)")
    test("PUT", f"/api/videos/{vid}/transcript", token,
         json_body={"segments": []}, expect=404,
         label=f"/api/videos/{{id}}/transcript (update no video)")
    test("POST", f"/api/videos/{vid}/transcript/regenerate", token, expect=404,
         label=f"/api/videos/{{id}}/transcript/regenerate (no video)")
    test("GET", f"/api/videos/{vid}/transcript/export-srt", token, expect=404,
         label=f"/api/videos/{{id}}/transcript/export-srt (no video)")
    test("POST", f"/api/videos/{vid}/transcript/import-srt", token, expect=[404, 422],
         label=f"/api/videos/{{id}}/transcript/import-srt (no video)")

    # ── Video ZIP export (no video) ──
    print("\n── Video Export ──")
    test("GET", f"/api/videos/{vid}/export", token, expect=404,
         label="/videos/{id}/export (no video)")

    # ── Clips (no video) ──
    print("\n── Clips ──")
    test("GET", "/api/clips", token, params={"video_id": vid}, expect=[200, 404],
         label="/clips?video_id= (no video)")
    test("GET", f"/api/clips/{uuid.uuid4()}", token, expect=404)
    test("PATCH", f"/api/clips/{uuid.uuid4()}", token,
         json_body={"title": "test"}, expect=404)
    ...
    # ── Publish (no clip) ──
    print("\n── Publishing ──")
    test("POST", "/api/publish/clip", token, json_body={
        "clip_id": str(uuid.uuid4()), "platform": "youtube_shorts",
        "access_token": "test-token",
    }, expect=[404, 422], label="/publish/clip (no clip)")

    # ── Webhooks (no video) ──
    vid2 = str(uuid.uuid4())
    test("GET", f"/api/publish/webhook/{vid2}", token,
         label="/publish/webhook/{id} (list)")
    test("POST", "/api/publish/webhook", token, json_body={
        "video_id": vid2, "url": "https://example.com/hook",
    }, expect=[200, 404], label="/publish/webhook (register)")
    test("DELETE", f"/api/publish/webhook/{vid2}", token, params={"url": "https://example.com/hook"},
         expect=[200, 404], label="/publish/webhook/{id} (unregister)")

    # ── Schedule (no clip) ──
    print("\n── Schedule ──")
    test("GET", "/api/schedule", token)
    test("POST", "/api/schedule", token, json_body={
        "clip_id": str(uuid.uuid4()), "platform": "youtube_shorts",
        "access_token": "test-token",
        "scheduled_at": "2030-01-01T00:00:00Z",
    }, expect=[404, 422], label="/schedule (create — no clip)")
    test("DELETE", f"/api/schedule/{uuid.uuid4()}", token, expect=404)

    # ── Research ──
    print("\n── Research ──")
    test("GET", "/api/research/trends", token, params={"source": "google", "geo": "US"})
    test("POST", "/api/research/analyze", token,
         json_body={"topic": "AI technology", "tone": "viral"})
    test("POST", "/api/research/crawl", token,
    ...
    # ── Clip dub (no clip) ──
    print("\n── Clip Dub ──")
    test("POST", f"/api/clips/{uuid.uuid4()}/dub", token, json_body={"target_lang": "es"}, expect=404)
    ...
    if r.status_code == 200:
        ...
        if vr.status_code == 200:
            for v in vr.json().get("items", []):
                httpx.delete(f"{API}/videos/{v['id']}", headers={"Authorization": f"Bearer {t}"}, timeout=10)
